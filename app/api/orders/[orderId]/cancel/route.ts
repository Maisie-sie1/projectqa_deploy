...
export async function POST(
  ...
  try {
    ...
    if (!["pending", "paid"].includes(order.status)) {
      return Response.json({ error: "ไม่สามารถยกเลิกคำสั่งซื้อในสถานะนี้ได้" }, { status: 400 });
    }
    ...
    return Response.json({ success: true, message: "ยกเลิกคำสั่งซื้อเรียบร้อยแล้ว" });
  } catch (e: any) {
    return Response.json({ error: e.message || "เกิดข้อผิดพลาดในการยกเลิกคำสั่งซื้อ" }, { status: 500 });
  }
}
